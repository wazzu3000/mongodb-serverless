...
export function model(name: string, rules?: Rules | undefined) {
  return function (constructor: Function) {
    ...
    if (_rules.access.getOne != 'disabled') {
      ...
      if (typeof _rules.access.getOne === 'object' || _rules.access.getOne == 'private') {
        app.get(url, handleAuthRequest);
      }
    ...
    if (_rules.access.getMany != 'disabled') {
      const url = `/api/${name}`;
      if (typeof _rules.access.getMany === 'object' || _rules.access.getMany == 'private') {
        app.get(url, handleAuthRequest);
      }
    ...
    if (_rules.access.post != 'disabled') {
      const url = `/api/${name}`;
      if (typeof _rules.access.post === 'object' || _rules.access.post == 'private') {
        app.post(url, handleAuthRequest);
      }
    ...
    if (_rules.access.put != 'disabled') {
      const url = `/api/${name}/:id`;
      if (typeof _rules.access.put === 'object' || _rules.access.put == 'private') {
        app.put(url, handleAuthRequest);
      }
    ...
    if (_rules.access.delete != 'disabled') {
      const url = `/api/${name}/:id`;
      if (typeof _rules.access.delete === 'object' || _rules.access.delete == 'private') {
        app.delete(url, handleAuthRequest);
      }
